...
describe("routes : flairs", () => {
  beforeEach(done => {
    ...
    sequelize.sync({force: true}).then(res => {
      //#1
      Topic.create({title: "Winter Games", description: "Post your Winter Games stories."}).then(topic => {
        this.topic = topic;

        Post.create({title: "Snowball Fighting", body: "So much snow!", topicId: this.topic.id}).then(post => {
          this.post = post;

          Flair.create({name: "I like it", color: "green", postId: this.post.id}).then(flair => {
            this.flair = flair;
            done();
          }).catch(err => {
            console.log(err);
